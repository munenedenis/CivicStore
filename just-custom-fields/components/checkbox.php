	...
	function field( $args ) {
		...
		echo '<div class="checkbox-row">';
			foreach( (array) $values as $key => $val ) {
				...
				echo '<label><input type="checkbox" ';?> onclick="showMe('jcf_field-inputtext-<?php echo $val?>', this)" <?php echo 'name="'.$this->get_field_name('val'). ($single_checkbox ? '' : '[]') . '" id="'.$this->get_field_id('val').'" value="'.esc_attr($val).'" '.checked(true, $checked, false).'/> '.$key.'</label>' . "\n";
			}
		echo '</div>';
		echo '</div>';
